...
- 📝 **Obsidian 兼容**: 输出标准 Markdown 图片链接，并兼容支持 LaTeX 的 Markdown 阅读器
...
默认模式会完整核对收藏夹分页。收藏夹分页响应自带每项内容的更新时间（与回答/专栏 API 同源同值），已有清单且该时间戳未变化、本地哈希及图片均正常的回答会零网络请求快速跳过；缺少清单、源内容已更新、本地文件被修改或图片缺失时会重新验证并按需修复。
